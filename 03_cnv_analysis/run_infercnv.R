...
#
# Usage:
#   Rscript run_infercnv.R <sample_path> <sample_name> <gene_order_file> [annotation_file]
#
# Arguments:
...
#                                                       barcode + final_annotation,
#                                                       written by annotate_sample.py)
#   sample_name      Sample identifier (used to locate the cell annotation
#                     file and to name outputs).
#   gene_order_file  Path to the inferCNV gene ordering file (chromosome, start,
#                     stop per gene). This study used GENCODE v27 (hg38); see
#                     resources/README.md.
#   annotation_file  (Optional) Path to the cell annotation file. Defaults to
#                     <sample_path>/<sample_name>_cell_annotation.txt
#
# Output:
...
#
# Notes:
#   - Reference cells are chosen from the annotation written by
#     annotate_sample.py: T cells where available, otherwise any non-malignant,
#     non-stromal cell type.
...
if (length(args) < 3) {
  stop(
    "Usage: Rscript run_infercnv.R <sample_path> <sample_name> <gene_order_file> [annotation_file]\n",
    "  sample_path      Path to the directory with the sample's analyzed data\n",
    "  sample_name      Sample identifier\n",
    "  gene_order_file  Path to the inferCNV gene order file\n",
    "  annotation_file  Optional path to the cell annotation file\n",
    call. = FALSE
  )
}
...
annotation_path <- if (length(args) >= 4) {
  args[4]
} else {
  file.path(sample_path, paste0(sample_name, "_cell_annotation.txt"))
}
...
# Cell annotation file from annotate_sample.py
# (tab-separated, no header: cell barcode, final_annotation).
...
# threads: override with INFERCNV_THREADS if set
num_threads <- as.integer(Sys.getenv("INFERCNV_THREADS", unset = "40"))
